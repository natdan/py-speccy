...
class RegistersGUI(Collection):
    def __init__(self, rect: Rect, ui_factory: BaseUIFactory, z80: Z80) -> None:
        ...
        self.second_row_label = ui_factory.label(Rect(rect.x, rect.y + 20, 0, 0), "")
        self.third_row_label = ui_factory.label(Rect(rect.x, rect.y + 40, 0, 0), "")
    ...
    def draw(self, surface) -> None:
        self.first_row_label.text = (
            ...
            f"PC:0x{self.z80.regPC:04x} "
            f"SP:0x{self.z80.regSP:04x} "
            f"OP:0x{self.z80.memory.peekb(self.z80.regPC):02x}({self.z80.memory.peekb(self.z80.regPC):03d}) ")
        self.second_row_label.text = (
            f"A:0x{self.z80.regA:02x} "
            f"HL:0x{self.z80.get_reg_HL():04x} "
            f"BC:0x{self.z80.get_reg_BC():04x} "
            f"DE:0x{self.z80.get_reg_DE():04x} "
            f"F:0x{self.z80.get_flags():02x} "
